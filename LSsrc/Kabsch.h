...
{	
	...
	if (spur>0)
	{
		...
		//compute b anyway
		if (a_failed != 1) //a is computed correctly
		{
			//compute b
			for (l = 0; l<2; l++)
			{
...
---------------------------------------------------------------------------
x    - x(i,m) are coordinates of atom m in set x            (input)
sxInd- the start index in x
exInd- the end index in x
y    - y(i,m) are coordinates of atom m in set y            (input)
syInd- the start index in y
eyInd- the end index in y
u    - u(i,j) is   rotation  matrix for best superposition  (output)
t    - t(i)   is translation vector for best superposition  (output)
**************************************************************************/
bool Kabsch(vector<Point3D> x,
	int sxInd,
	int exInd,	// [sxInd, exInd)
	vector<Point3D> y,
	int syInd,
	int eyInd,	// [syInd, eyInd)
	double t[3],
	double u[3][3]
	)
{	
	int xlen = exInd - sxInd;
	int ylen = eyInd - syInd;
